...
export default function AcercaSection() {
    ...
    const openPDF = () => {
        window.open("/CV_Lorenzo_Noe_Chavez_Cerda_Web_Developer.pdf", "_blank");
    };
        ...
        <Box display={"flex"} h={"100vh"} w={"100%"} flexDirection={flexDirection} p={paddingFlex} alignItems={"center"} justifyContent={justifyContentResponsive} id="Acerca">
            ...
            <Box backgroundColor={theme.colors.secondaryDark} w={'auto'} maxW={sizeResponsive} h={'auto'} p={"1rem"} borderRadius={"2rem"} animation={"shadowColorAnimated 1s ease-in-out infinite"}>
                ...
                <Flex justifyContent={"center"} gap={"2rem"} marginTop={".5rem"} >
                    <Button onClick={scrollToComponent} resource="" leftIcon={<EmailIcon />}  backgroundColor={theme.colors.dark} color={theme.colors.highlight} boxShadow={"2px 2px 5px black"} _hover={{boxShadow: `2px 2px 5px ${theme.colors.highlight}`}}>Contrátame</Button>
                    <Button onClick={openPDF} leftIcon={<AttachmentIcon />} backgroundColor={theme.colors.dark} color={theme.colors.highlight} boxShadow={"2px 2px 5px black"} _hover={{boxShadow: `2px 2px 5px ${theme.colors.highlight}`}}>CV</Button>
                </Flex>
            </Box>
        </Box>
    )
